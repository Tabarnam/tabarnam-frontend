...
export function buildCompanyDraft(company) {
  ...
  // Pre-fill a name-based Amazon brand search when no URL exists yet (and the
  // company isn't marked "no store"), so every editor-open path — companies
  // list, import review, post-save rebuild — shows a candidate Amazon URL to
...
// Phase 2.19.B — count product keywords, handling both array and
...
export function toIssueTags(company) {
  // Issues column renders from enrichment_health.missing_fields (contract), not legacy heuristics.
  // Legacy heuristics treat placeholders like "Unknown" as present and hide real missing fields.
  const tags = getContractMissingFields(company);

  // Phase 2.19.B — partial-products signal.
  //
  // When the products field is populated but sparse (fewer than
  // PARTIAL_PRODUCTS_THRESHOLD entries), surface a "+ products" chip in
  // the Issues column. This tells the admin "products has SOMETHING but
  // may not be the full list — worth a manual review." Distinct from
  // the "products" missing tag which means "no products at all."
  //
  // Skip the partial tag when products is ALREADY flagged as fully
  // missing — no point adding a partial signal for a completely empty
  // field.
