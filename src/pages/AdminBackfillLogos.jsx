...
export default function AdminBackfillLogos() {
  ...
  const kickWorker = useCallback((jobId) => {
    ...
    try {
      const url = join(API_BASE, "xadmin-api-backfill-logos-worker");
      fetch(url, {
        method: "POST",
  ...
  const fetchStatus = useCallback(async () => {
    try {
      const res = await apiFetch("/xadmin-api-backfill-logos-status");
  ...
  const startBackfill = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiFetch("/xadmin-api-backfill-logos-start", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const sendAction = useCallback(async (action) => {
    ...
    try {
      const res = await apiFetch(`/xadmin-api-backfill-logos-status?action=${action}&job_id=${encodeURIComponent(jobId)}`, { method: "POST" });
      const data = await readJsonOrText(res);
      if (!res.ok) throw new Error(data?.error || `HTTP ${res.status}`);
  ...
  return (
    <>
      <Helmet><title>Admin - Backfill Logos</title></Helmet>
      <AdminHeader />

      <div className="bg-slate-950 min-h-screen p-6">
        <div className="max-w-[1200px] mx-auto">
          <h1 className="text-2xl font-bold text-white mb-1">Backfill Logos</h1>
          <p className="text-slate-400 text-sm mb-6">
            Fetch company logos via Microlink for any company that's missing a logo or has an unapproved one.
            Backfilled logos land <span className="font-semibold">unapproved</span> — review them in
            <a href="/admin/images" className="text-teal-400 hover:underline mx-1">/admin/images</a>
            (or <a href="/admin/logos" className="text-teal-400 hover:underline">/admin/logos</a>) before they go public.
            Manually-uploaded blob logos are protected by a merge guard and will not be overwritten.
          </p>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
            ...
            <div className="rounded-lg border border-emerald-900/50 bg-emerald-950/30 p-3">
              <div className="text-xs uppercase text-emerald-300/80 tracking-wider">With logo</div>
              <div className="text-2xl font-semibold text-emerald-300">{status?.companies_with_logo?.toLocaleString() ?? "—"}</div>
            </div>
            <div className="rounded-lg border border-amber-900/50 bg-amber-950/30 p-3">
              ...
              <div className="text-2xl font-semibold text-amber-300">{status?.companies_missing_logo?.toLocaleString() ?? "—"}</div>
            </div>
