...
export async function readJsonOrText(res: Response): Promise<unknown> {
  ...
  // Phase 4.4 — detect HTML responses (typically 502/503/504 gateway pages
  // returned by Azure Front Door, App Service, or any intermediate proxy
  // when the backend Function App is restarting / scaling / cold-starting).
  // We were previously returning `{ text: "<!DOCTYPE html>..." }` which
  // bled raw HTML into UI status panels (observed 2026-05-12 during a
  // Phase 4.3 deploy when Azure restarted the Function App). Detect both
  // the content-type header AND a sniff of the body, and return a soft
  // gateway-error object instead. Preserves status code for callers that
  // want to surface a custom retry message.
  const looksLikeHtml = isHtml || /^\s*<(?:!doctype|html|head|body)/i.test(text);
  if (looksLikeHtml) {
    const status = typeof cloned.status === "number" ? cloned.status : 0;
    return {
      error: "gateway_html_response",
      status,
      message:
        status >= 500 && status < 600
          ? "Backend temporarily unavailable (gateway returned HTML)."
          : "Unexpected HTML response from backend.",
    };
  }
...
export async function apiFetch(path: string, init?: RequestInit) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      // NOTE: apiFetch prod-base fallback for refresh endpoints was REMOVED.
      // It caused ghost locks: the fallback fires immediately on SWA 500 and acquires
      // a backend lock, but SWA may have already killed the connection — leaving an
      // orphaned lock in Cosmos that blocks subsequent refresh attempts.
      // SWA 500 retries are now handled at the caller level (CompanyDashboard.jsx)
      // with proper delays, backoff, and user-facing feedback.
