...
const PROMPT_GUIDANCE_VERSION = "7.0.2-no-strict-schema";
...
function buildCanonicalImportPrompt({
  ...
  return `You are a precise company research specialist. Use web_search and browse_page tools aggressively. Cross-verify every fact with at least 3 independent sources. Do not hallucinate. Focus only on the specified company.

For the Company: ${name} / ${url}
Fields to populate: ${fieldList}

Follow these baked-in rules for accuracy and formatting:
Be thorough, exact, verify sources using web_search and browse_page tools, and do not hallucinate. Focus only on the specified company.

Tagline: Provide the company's tagline, slogan, or motto. Format as described below.

HQ: Conduct thorough research using web_search and browse_page tools to identify the HQ location, cross-verifying across at least 3 independent sources (e.g., official website, company profiles like LinkedIn or Crunchbase, and recent articles or filings) and resolving any discrepancies. Use initials for states or provinces (e.g., City, State Initials, Country). Use USA, not US. No explanatory info — just the location. If multiple HQ locations, separate with semicolons. Format: HQ: City, ST, Country or HQ: City, ST, Country; City2, ST2, Country2

Manufacturing: Conduct thorough research using web_search and browse_page tools to identify all known manufacturing locations worldwide, cross-verifying across at least 3 independent sources (e.g., official website, company profiles like LinkedIn or Crunchbase, and recent articles or filings) and resolving any discrepancies. Include every city and country found, with a deep dive on any US sites to confirm actual cities. List them exhaustively without missing any. Use initials for states or provinces. Use USA, not US. No explanatory info — just the locations. If part of a location is unspecified, include only what is known. Do not write "unspecified." Separate each location with semicolons. Format: Manufacturing: City, ST, Country; City2, ST2, Country2

Industries: Use web_search "[Company Name] [Website URL] industry" or "[Company Name] company profile" to find verified products, categories, and offerings. Return 8-15 short noun phrases (1-3 words preferred) that read like actual user search terms for what the company manufactures or sells. Mix broad categories and specific sub-types (e.g., "Beef Jerky", "Snack Foods", "Dried Meats", "Hawaiian Foods"). Use normalized, common-search-term language. Include meaningful product distinctions ("organic", "leather", "stainless steel") but avoid narrative/marketing adjectives ("artisan", "premium", "hand-crafted"). Do NOT return generic umbrella terms like "Consumer Goods", "Food and Beverage", "Retail", "E-Commerce", "Food", "Shopping". Only verified information; no guessing. Format: Industries: industry1, industry2, industry3

Products: Exhaustive, complete list of all products the company produces, separated by commas. Format: Products: product1, product2, product3

Reviews: Find 5 unique, legitimate third-party reviews with working URLs. Use 1-2 YouTube reviews focused solely on the current company or its products; do not include unrelated reviews or reviews from or about previously discussed companies. The remaining reviews should be from X (Twitter), a magazine or blog, strictly related to the current company and its products, excluding any overlap with prior companies. Confirm all URLs are functional. Do not hallucinate or embellish. Do not include the same author or URL more than once. Accuracy is paramount. For a YouTube video, YouTube is the source. Separate each review with one blank line. Output each review in this exact plain-text format:
Source: [Name of publication, channel, or website]
Author: [Author or channel name]
URL: [Direct URL to the review/article/video, not the site root]
Title: [Exact title as published]
...
If you cannot verify a field after exhaustive search and your full tool budget, return it as the appropriate empty value ("" for strings, [] for arrays, false for booleans) — do not fabricate, omit, or hallucinate any required field. After 5 web_search or browse_page tool calls you MUST stop searching and emit the JSON object with empty values for any field still unverified.

Return ONLY a single JSON object with these exact property names: tagline, headquarters_location, manufacturing_locations, industries, product_keywords, reviews, location_source_urls (with hq_source_urls and mfg_source_urls), red_flag, social. No prose, no markdown, no extra text whatsoever. For the fields that have specific formatting rules above (tagline, headquarters_location, manufacturing_locations, industries, product_keywords, reviews), follow those formatting rules precisely when populating the corresponding JSON values.`;
}
