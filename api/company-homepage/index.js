...
function getStorageCredentials(ctx) {
  ...
  ctx.log(`[company-homepage] Configured account: ${accountName}`);
  ctx.log(`[company-homepage] Account key present: ${!!accountKey}`);
...
async function companyHomepageHandler(req, ctx) {
  if (req.method === "OPTIONS") return { status: 200, headers: cors(req) };

  try {
    let { accountName, accountKey } = getStorageCredentials(ctx);
    if (!accountKey) {
      return json({ ok: false, error: "Server storage not configured.", accountName }, 500, req);
    }
    ...
    if (!blobName) {
      return json(
        { ok: false, error: "Missing or invalid src. Expected an Azure blob URL under company-homepages, or a blob path like <companyId>/<file>." },
        400,
        req
      );
    }
    ...
    if (!sdk) {
      return json({ ok: false, error: "Server dependency missing. Please ensure @azure/storage-blob is installed." }, 500, req);
    }
    ...
    try {
      ...
      if (status === 404) {
        return {
          status: 404,
          headers: { ...cors(req), "Cache-Control": "public, max-age=60", "Content-Type": "text/plain; charset=utf-8" },
          body: "Not found",
        };
      }
      ctx.error(`[company-homepage] download failed for ${blobName}: ${e?.message || e}`);
      return json({ ok: false, error: "Failed to fetch homepage image." }, 500, req);
    }
    ...
    if (etag && ifNoneMatch && ifNoneMatch === etag) {
      return {
        status: 304,
        headers: { ...cors(req), ETag: etag, "Cache-Control": "public, max-age=86400, stale-while-revalidate=604800" },
      };
    }
    ...
    ctx.error(`[company-homepage] Unexpected error: ${e?.message || e}`);
    return json({ ok: false, error: "Unexpected error." }, 500, req);
  }
}

app.http("company-homepage", {
  route: "company-homepage",
  methods: ["GET", "OPTIONS"],
  authLevel: "anonymous",
  handler: companyHomepageHandler,
});

module.exports = { handler: companyHomepageHandler };
