...
const { handler: suggestStatesHandler } = require("./index.js");
...
function makeReq({
  url = "https://example.test/api/suggest-states",
  method = "GET",
  headers,
} = {}) {
...
test("suggestStatesHandler is exported correctly", () => {
  assert.equal(typeof suggestStatesHandler, "function");
});
...
test("OPTIONS returns 200 with CORS headers", async () => {
  ...
  const res = await suggestStatesHandler(req, ctx);
...
test("GET without q param returns empty suggestions", async () => {
  const req = makeReq({ url: "https://example.test/api/suggest-states" });
  const res = await suggestStatesHandler(req, ctx);
...
test("GET with q param returns valid response shape (graceful without Cosmos)", async () => {
  const req = makeReq({
    url: "https://example.test/api/suggest-states?q=test&country=CA",
  });
  const res = await suggestStatesHandler(req, ctx);
  const body = parseJson(res);
