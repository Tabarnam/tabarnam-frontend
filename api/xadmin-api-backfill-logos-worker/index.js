// Backfill logos — WORKER endpoint.
// Mirrors xadmin-api-backfill-homepages-worker. Claims the job lock, runs one
// bounded invocation of processBackfillLogosBatch (parallel waves of Microlink
// logo fetches), releases the lock, returns. Status endpoint self-drives
// further invocations on each poll.
...
async function releaseLock({ jobsContainer, jobId, workerId, context }) {
  ...
  } catch (e) { context.log(`[backfill-logos-worker] releaseLock failed: ${e?.message || e}`); }
}
...
async function handler(req, context) {
  ...
  const workerId = `logos-worker-${Math.random().toString(36).slice(2, 10)}-${Date.now().toString(36)}`;

  let processBackfillLogosBatch;
  try {
    ({ processBackfillLogosBatch } = require("../xadmin-api-backfill-logos-start/index.js"));
  } catch (e) {
    return json({ ok: false, error: "batch_processor_unavailable", detail: e?.message || String(e) }, 500);
  }

  const startedAt = Date.now();
  context.log(`[backfill-logos-worker] start job=${jobId} worker=${workerId} budget=${(invocationBudgetMs / 1000).toFixed(0)}s`);

  const claim = await tryClaimLock({ jobsContainer, jobId, workerId });
  if (!claim.claimed) {
    context.log(`[backfill-logos-worker] lock_not_claimed job=${jobId} reason=${claim.reason}`);
    return json({ ok: true, job_id: jobId, claimed: false, reason: claim.reason, job_status: claim.job?.status || null });
  }

  let result;
  try {
    result = await processBackfillLogosBatch({ session_id: jobId, requested_by: "logos_worker", invocationBudgetMs }, context);
  } catch (e) {
  ...
  context.log(`[backfill-logos-worker] exit job=${jobId} processed=${result?.processed ?? "?"} failed=${result?.failed ?? "?"} remaining=${result?.remaining ?? "?"} elapsed=${(elapsedMs / 1000).toFixed(1)}s exit=${result?.exit_reason || result?.error || "?"}`);
...
app.http("adminBackfillLogosWorker", {
  route: "xadmin-api-backfill-logos-worker",
