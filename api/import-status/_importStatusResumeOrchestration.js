...
async function runWatchdogStuckDetection(ctx, opts) {
  ...
  try {
    ...
    // Phase 3.2 — watchdog for stuck "in_progress" workers.
    //
    // Background: when import-status triggers a resume-worker via HTTP
    // self-invocation, it marks the resume doc as status:"in_progress"
    // (line 552). The re-trigger gate (line 516) only allows re-triggering
    // when status is queued/blocked/error/stalled — NOT in_progress. So if
    // the worker SILENTLY DIES (Azure host recycle, OOM, fetch hanging on
    // a closed connection), the resume doc stays "in_progress" until some
    // other path resurrects it. The OLD synchronous import-start
    // architecture didn't have this gap because all work happened in one
    // Function invocation — no "in_progress" state to get stuck in.
    //
    // Empirical (Clabber + Duncan, 2026-05-10): worker died around
    // 02:04:02, but import-status didn't re-trigger until 02:07:36 — a
    // 3.5-minute gap with no in_progress watchdog covering it. The xAI
    // lock TTL eventually expired, but only after another worker happened
    // to be polling — which never happens at XAI_CONCURRENCY=1 once the
    // single active worker dies.
    //
    // The worker writes resume_worker_heartbeat_at periodically (every
    // ~10-30s via Phase 3.1.1 + maybeWriteHeartbeat). If we see status
    // in_progress AND no heartbeat for >60s, the worker is dead. Flip
    // status to "stalled" so the next trigger gate fires.
    const lastHeartbeatTs = Date.parse(
      String(sessionDocForWatchdog?.resume_worker_heartbeat_at || "")
    ) || 0;
    const lastEnteredForHbTs = Date.parse(
      String(sessionDocForWatchdog?.resume_worker_handler_entered_at || "")
    ) || 0;
    const inProgressStatus = String(ctx.resumeStatus || "").toLowerCase();
    const heartbeatStaleThresholdMs = 60_000;
    const referenceTs = Math.max(lastHeartbeatTs, lastEnteredForHbTs);
    if (
      ctx.resume_needed &&
      (inProgressStatus === "in_progress" || inProgressStatus === "running") &&
      referenceTs &&
      Date.now() - referenceTs > heartbeatStaleThresholdMs
    ) {
      const staleAt = nowIso();
      const staleAgeMs = Date.now() - referenceTs;
      ctx.stageBeaconValues.status_resume_watchdog_in_progress_stale = staleAt;
      ctx.stageBeaconValues.status_resume_watchdog_in_progress_stale_age_ms = staleAgeMs;
      console.warn(`[import-status] phase_3_2_in_progress_watchdog_fired`, {
        session_id: ctx.sessionId,
        prior_status: inProgressStatus,
        heartbeat_age_ms: lastHeartbeatTs ? Date.now() - lastHeartbeatTs : null,
        entered_age_ms: lastEnteredForHbTs ? Date.now() - lastEnteredForHbTs : null,
        threshold_ms: heartbeatStaleThresholdMs,
        action: "flip_status_to_stalled_so_re_trigger_can_fire",
      });
      // Flip ctx.resumeStatus to "stalled" so runResumeTriggerExecution's
      // gate at line 516 will fire on this very same import-status call.
      ctx.resumeStatus = "stalled";

      // Also persist to Cosmos so subsequent polls (and the resume worker
      // itself when it comes back via re-trigger) see the stalled state.
      try {
        const resumeDocId = `_import_resume_${ctx.sessionId}`;
        const resumeDocStale = await readControlDoc(ctx.container, resumeDocId, ctx.sessionId).catch(() => null);
        if (resumeDocStale && typeof resumeDocStale === "object") {
          await upsertDoc(ctx.container, {
            ...resumeDocStale,
            status: "stalled",
            stalled_reason: "in_progress_heartbeat_stale_watchdog",
            stalled_heartbeat_age_ms: staleAgeMs,
            stalled_at: staleAt,
            updated_at: staleAt,
          }).catch(() => null);
        }
      ...
      // Re-enqueue via the queue trigger as belt-and-suspenders. The
      // upcoming HTTP self-invocation in runResumeTriggerExecution will
      // get there first if the host is responsive; if not, the queue
      // message will fire when the host wakes up.
      try {
        ...
        await enqueueResumeRun({
          ...
          reason: "in_progress_heartbeat_stale_watchdog",
