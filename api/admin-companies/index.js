  ...
  handler: async (req, context) => {
    ...
    try {
      ...
      if (method === "POST" || method === "PUT") {
        ...
        context.log(`[admin-companies] Upserting company:`, { id: partitionKeyValue, method, nameCheck: doc.company_name });

        try {
          const result = await container.items.upsert(doc, { partitionKey: partitionKeyValue });
          context.log(`[admin-companies] Upsert success:`, { id: partitionKeyValue, statusCode: result.statusCode, resourceId: result.resource?.id });
          return json({ ok: true, company: doc }, 200);
        } catch (e) {
          context.log("[admin-companies] Upsert error:", { id: partitionKeyValue, error: e?.message, code: e?.code, stack: e?.stack });
