...
export default app.http('adminCompanies', {
    ...
    if (method === "DELETE") {
      ...
      try {
        ...
        if (!documentNormalizedDomain || String(documentNormalizedDomain).trim() === "") {
          ...
          context.log("[admin-companies] DELETE: Document missing normalized_domain, computed from URL", {
            id: doc.id,
            company_name: doc.company_name,
        ...
        if (!documentNormalizedDomain || String(documentNormalizedDomain).trim() === "" || String(documentNormalizedDomain).trim() === "unknown") {
          context.log("[admin-companies] DELETE ERROR: Cannot determine normalized_domain for document", {
            id: doc.id,
            company_name: doc.company_name,
        ...
        context.log("[admin-companies] DELETE: Partition key ready", {
          itemId: doc.id,
        ...
        try {
          ...
          try {
            ...
            context.log("[admin-companies] SOFT DELETE: Attempting fallback upsert without explicit partition key parameter...");
            try {
