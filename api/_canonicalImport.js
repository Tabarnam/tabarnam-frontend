 ...
 * we already ran often contains tagline + HQ visible on the page. This
 * helper makes a SHORT, bounded xAI call with NO tools enabled to extract
 * just those two fields from the prefetched homepage text. Pure
 * summarization, no web_search, no tool-loop possible.
 *
 * Empirical (2026-05-09 batch — Santa Cruz Organic / Spread the Love /
 * MaraNatha / Adams): all four failed canonical calls had non-empty
 * homepage prefetch contexts (>100 chars) containing the tagline and
 * city-level HQ. Without this fallback, those fields landed empty and
 * the row showed Stub-0%. With the fallback, those rows can land
 * Partial-X% with at minimum tagline + HQ populated.
 *
 ...
 * (either may be empty string if not findable in the prefetch) plus
...
async function tryHomepageExtractionFallback({
  ...
  const fallbackPrompt = `You are extracting two specific facts from the company's homepage text below. Use ONLY the homepage text provided — do not search, do not make up information.
...
Extract:
- tagline: the company's tagline, slogan, mission statement, or brand promise as it appears on the homepage. If the homepage has no clear tagline, emit "".
- headquarters_location: the city, state/province, and country of the company's headquarters if visible in the homepage text (often in a footer, contact section, or about paragraph). Format as "City, ST, USA" or "City, Country". If not visible, emit "".

Output ONLY a JSON object with exactly these two keys: tagline (string), headquarters_location (string). No markdown, no prose, no extra text.`;
  ...
  try {
    const res = await xaiLiveSearchStreaming({
      ...
      timeoutMs: 60_000,
      model: model || asString(process.env.XAI_MODEL).trim() || "grok-4-latest",
...
async function buildFailureOrPartialFromFallback({
  ...
  if (fallback && (fallback.tagline || fallback.headquarters_location)) {
    // Partial success: at least tagline or HQ was extractable.
    const fields = Array.isArray(fieldsToEnrich) && fieldsToEnrich.length ? fieldsToEnrich : DEFAULT_CANONICAL_FIELDS;
    const completed = [];
    const failed = [];
    const errors = {};
...
function mergeMultiCallResults({ coreResult, locationsResult, reviewsResult }) {
  ...
  return {
    tagline: stripLabel(asString(core?.tagline)),
    headquarters_location: stripLabel(asString(loc?.headquarters_location)),
    manufacturing_locations: Array.isArray(loc?.manufacturing_locations)
      ? loc.manufacturing_locations.map((s) => asString(s)).filter(Boolean)
      : [],
    industries: Array.isArray(core?.industries)
      ? core.industries.map((s) => asString(s)).filter(Boolean)
      : [],
    product_keywords: asString(core?.product_keywords),
...
async function runMultiCallCanonical({
  ...
  // Fire all 3 in parallel via Promise.allSettled.
  const [coreSettled, locationsSettled, reviewsSettled] = await Promise.allSettled([
    _runMultiSubCall({
      callName: "core",
      prompt: corePrompt,
      model,
  ...
  // Promise.allSettled never rejects — unwrap to access the sub-call results.
  // If a sub-call's promise itself rejected (vs returned ok:false), synthesize
  // a failure result for it.
  const coreResult = coreSettled.status === "fulfilled" ? coreSettled.value : {
    ok: false,
    parsed: null,
    error_code: "promise_rejected",
    diagnostics: { call_name: "core", error: String(coreSettled.reason?.message || coreSettled.reason) },
    raw_text: "",
  };
  const locationsResult = locationsSettled.status === "fulfilled" ? locationsSettled.value : {
    ok: false,
    parsed: null,
    error_code: "promise_rejected",
    diagnostics: { call_name: "locations", error: String(locationsSettled.reason?.message || locationsSettled.reason) },
    raw_text: "",
  };
  const reviewsResult = reviewsSettled.status === "fulfilled" ? reviewsSettled.value : {
    ok: false,
    parsed: null,
    error_code: "promise_rejected",
    diagnostics: { call_name: "reviews", error: String(reviewsSettled.reason?.message || reviewsSettled.reason) },
    raw_text: "",
  };
