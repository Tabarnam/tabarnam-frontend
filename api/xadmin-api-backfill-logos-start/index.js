...
async function fireBatchWorker(jobId, logger) {
  ...
  const url = require("../_internalJobAuth").buildSelfInvokeUrl("/api/xadmin-api-backfill-logos-worker");
 ...
 * (auto-triggered after companies are saved). The merge guard in isPending()
 * ensures companies whose logos already imported successfully via the
 * inline import pipeline (logo_stage_status === "ok" | "imported") are
 * skipped automatically — no risk of clobbering verified blobs.
 *
 * Returns:
 ...
 */
async function createLogoBackfillJob(options = {}) {
  ...
  if (totalPending === 0) {
    return { ok: true, total_to_process: 0, message: "No companies need logo backfill" };
  }
...
async function handler(req, context) {
  ...
  const result = await createLogoBackfillJob({
...
async function processBackfillLogosBatch(queueBody, context) {
  ...
  if (shouldHandoff) {
    context.log(`[backfill-logos] handoff: re-firing worker for job=${jobId} remaining=${job.remaining}`);
