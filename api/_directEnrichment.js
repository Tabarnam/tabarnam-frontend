...
async function applyEnrichmentToCompany(company, enrichmentResult) {
  if (!company || !enrichmentResult?.enriched) {
    // Phase 2.9 A — log when this no-ops, so we can spot canonical results
    // arriving with empty/missing enriched objects (which would mean the
    // upstream call effectively failed even if the diagnostic claimed ok).
    try {
      console.warn(`[applyEnrichmentToCompany] no_op_skip`, {
        has_company: !!company,
        has_enrichment_result: !!enrichmentResult,
        has_enriched: !!enrichmentResult?.enriched,
        ok_flag: enrichmentResult?.ok,
  ...
  // Phase 2.9 A — diagnostic entry log so we can correlate post-hoc whether
  // applyEnrichmentToCompany ran at all + what shape it received. Empirical
  // (Earth Runners session bbefd04c, 2026-05-09): canonical call returned
  // ok:true with envelope shape but the cosmos doc lacked all the fields
  // this helper is supposed to produce (cleared *_unknown flags, *_status
  // audit fields, geocoded HQ, plural headquarters_locations array). Logging
  // tells us whether it ran and what each per-field block did with the
  // input.
  try {
    const fieldShapes = {};
    for (const f of ["tagline", "headquarters_location", "manufacturing_locations", "industries", "product_keywords", "reviews"]) {
      const v = enriched[f];
      if (v == null) {
        fieldShapes[f] = "missing";
      } else if (typeof v === "string") {
        fieldShapes[f] = `string(${v.length})`;
      } else if (Array.isArray(v)) {
        fieldShapes[f] = `array(${v.length})`;
      } else if (typeof v === "object") {
        fieldShapes[f] = `object{${Object.keys(v).join(",")}}`;
      } else {
        fieldShapes[f] = typeof v;
      }
    }
    console.log(`[applyEnrichmentToCompany] entered`, {
      company_id: company.id,
      company_name: company.company_name,
      ok_flag: enrichmentResult.ok,
      enriched_keys: Object.keys(enriched),
      field_shapes: fieldShapes,
      diagnostics_envelope_marker: enrichmentResult?.diagnostics?.enriched_envelope_shape ?? null,
      pre_hq_unknown: company.hq_unknown ?? null,
      pre_mfg_unknown: company.mfg_unknown ?? null,
    });
  } catch {}
  ...
  // Phase 2.9 A — diagnostic before processing so we can confirm whether
  // this block fires for the canonical envelope shape.
  try {
    console.log(`[applyEnrichmentToCompany] hq_block_check`, {
      company_id: company.id,
      enriched_hq_truthy: !!enriched.headquarters_location,
      enriched_hq_type: typeof enriched.headquarters_location,
      enriched_hq_inner_truthy: !!enriched.headquarters_location?.headquarters_location,
      enriched_hq_inner_value: typeof enriched.headquarters_location?.headquarters_location === "string"
        ? enriched.headquarters_location.headquarters_location.slice(0, 50)
        : null,
    });
  } catch {}
  if (enriched.headquarters_location) {
    ...
    if (enriched.headquarters_location.headquarters_location) {
      ...
      // Phase 2.9 A — log flag clearing so we can verify it ran
      try {
        console.log(`[applyEnrichmentToCompany] hq_flag_cleared`, {
          company_id: company.id,
          new_hq_unknown: updated.hq_unknown,
          new_hq_unknown_reason: updated.hq_unknown_reason,
          new_hq_status: updated.headquarters_location_status,
          hq_value: updated.headquarters_location.slice(0, 60),
        });
      } catch {}

      // Phase 2.19.D — HQ geocoding moved to a parallel block after the
      // MFG block (see below). Originally HQ geocoded synchronously here
      // before the MFG block ran, serializing the two geocode passes.
      // For multi-region brands (UGG: HQ in Goleta + 3 mfg sites) this
      // cost 10-20s of sequential geocoding. With both running via
      // Promise.all, total time = max(hq, mfg) instead of sum.
    }
  }

  // Apply manufacturing_locations
  // Phase 2.9 A — diagnostic before processing
  try {
    console.log(`[applyEnrichmentToCompany] mfg_block_check`, {
      company_id: company.id,
      enriched_mfg_truthy: !!enriched.manufacturing_locations,
  ...
  if (enriched.manufacturing_locations) {
    ...
    if (enriched.manufacturing_locations.manufacturing_locations) {
      ...
      // Phase 2.9 A — log flag clearing so we can verify it ran
      try {
        console.log(`[applyEnrichmentToCompany] mfg_flag_cleared`, {
          company_id: company.id,
          new_mfg_unknown: updated.mfg_unknown,
  ...
  // Phase 2.19.D — Parallel geocoding for HQ + manufacturing.
  //
  // Pre-2.19 the two geocode passes ran sequentially: HQ first
  // (synchronously inside the HQ block), then MFG (synchronously inside
  // the MFG block). For multi-region brands this serialized 5-25s of
  // network calls for no architectural reason. Now both kick off
  // concurrently via Promise.allSettled — total time becomes max(hq, mfg)
  // instead of sum. Each geocode is independently failure-tolerant
  // (failures are non-fatal; the field's status_unknown flag is
  // already set to false in the per-block logic above).
  {
    const hqString = typeof updated.headquarters_location === "string" ? updated.headquarters_location : null;
    const mfgArr = Array.isArray(updated.manufacturing_locations) ? updated.manufacturing_locations : null;

    const hqGeocodePromise = hqString
      ? geocodeLocationArray([{ address: hqString }], { timeoutMs: 5000, concurrency: 1 }).catch(() => null)
      : Promise.resolve(null);

    const mfgGeocodePromise = mfgArr && mfgArr.length > 0
      ? (() => {
          const seeds = mfgArr.map((loc) =>
            typeof loc === "string" ? { location: loc, address: loc } : loc
          );
          return geocodeLocationArray(seeds, { timeoutMs: 5000, concurrency: 4 }).catch(() => null);
        })()
      : Promise.resolve(null);

    const [hqGeocoded, mfgGeocoded] = await Promise.all([hqGeocodePromise, mfgGeocodePromise]);

    // Apply HQ geocoding result (back to the same shape the pre-2.19 HQ
    // block produced).
    if (hqString && Array.isArray(hqGeocoded) && hqGeocoded[0]?.lat != null && hqGeocoded[0]?.lng != null) {
      const geo = hqGeocoded[0];
    ...
    if (hqString) {
      try {
        console.log(`[applyEnrichmentToCompany] hq_geocode_done`, {
          company_id: company.id,
          hq_string: hqString.slice(0, 60),
          found_coords: !!(updated.hq_lat != null && updated.hq_lng != null),
          hq_lat: updated.hq_lat ?? null,
          hq_lng: updated.hq_lng ?? null,
          built_plural_array: Array.isArray(updated.headquarters_locations),
        });
      } catch {}
    }

    // Apply MFG geocoding result.
    if (Array.isArray(mfgGeocoded) && mfgGeocoded.length > 0) {
      updated.manufacturing_geocodes = mfgGeocoded;
    }
  }
  ...
  if (!(updated.rating?.star4?.value > 0)) {
    try {
      ...
      if (scoring.ok) {
        ...
        updated.rating.star4 = { ...existingStar4, value: scoring.reputation_score, reasoning: scoring.reputation_reasoning };
        updated.rating.star5 = { ...existingStar5, value: scoring.quality_score, reasoning: scoring.quality_reasoning };
        console.log(`scoring_call: duration=${((Date.now() - scoringStart) / 1000).toFixed(1)}s, star4=${scoring.reputation_score.toFixed(2)}, star5=${scoring.quality_score.toFixed(2)}, reasoning_populated=true`);
      } else {
  ...
  console.log(`[enrichment_final_state] company="${updated.company_name || updated.name || "(unknown)"}" domain=${updated.normalized_domain || "(unknown)"}`, {
    ...
    // Phase 4.3 — count both array and comma-separated string shapes. The
    // canonical pipeline writes product_keywords as a comma-separated string
    // per the JSON schema (`type: "string"`), but legacy code paths may write
    // an array. Without the string branch this always logged 0, masking
    // populated product lists in production diagnostics.
    product_keywords_count: typeof updated.product_keywords === "string"
      ? updated.product_keywords.split(",").filter((s) => s.trim()).length
      : Array.isArray(updated.product_keywords)
        ? updated.product_keywords.length
        : 0,
  ...
  // Phase 2.9 A — exit log so we can confirm the function completed and
  // returned the expected post-apply state. If this log fires but the
  // Cosmos doc still shows stale flags, the issue is upstream-of-upsert
  // (handler.js doesn't persist the returned doc). If this log doesn't
  // fire, the function threw silently somewhere.
  try {
    console.log(`[applyEnrichmentToCompany] exit`, {
      company_id: company.id,
      hq_unknown_after: updated.hq_unknown ?? null,
      mfg_unknown_after: updated.mfg_unknown ?? null,
      hq_status_set: !!updated.headquarters_location_status,
      mfg_status_set: !!updated.manufacturing_locations_status,
      tagline_status_set: !!updated.tagline_status,
      industries_status_set: !!updated.industries_status,
      hq_geocoded: updated.hq_lat != null && updated.hq_lng != null,
      headquarters_locations_built: Array.isArray(updated.headquarters_locations),
      manufacturing_geocodes_built: Array.isArray(updated.manufacturing_geocodes),
    });
  } catch {}
