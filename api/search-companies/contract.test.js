...
test("head-noun broadening pulls noun-makers into a multi-word category search", async () => {
  resetTypoCorrectionCache();
  resetIndustryAffinityCache();
  // Pass 1 AND's "custom"+"emblem" → only the both-word company. The head-noun
  // (emblem) supplemental query then pulls in the emblem-only maker.
  const bothWord = { id: "acme", company_id: "acme", company_name: "Acme Manufacturing", normalized_domain: "acme.com", keywords: ["custom emblem", "emblem", "badges"], search_tokens: ["custom", "emblem"], search_text_norm: "custom emblem badges", industries: ["Emblems"], _ts: 1700000000 };
  const emblemOnly = { id: "mainevent", company_id: "mainevent", company_name: "Main Event Emblems", normalized_domain: "mainevent.com", keywords: ["emblem", "emblems"], search_tokens: ["emblem"], search_text_norm: "emblem emblems", industries: ["Emblems"], _ts: 1700000001 };
  const container = makeContainer(async (spec) => {
    const q = String(spec?.query || "");
    if (q.includes("@hn")) return [emblemOnly, bothWord]; // head-noun (emblem) OR query
    if (q.includes("ARRAY_CONTAINS")) return [bothWord];  // Pass 1 (custom AND emblem)
    return [];
  });
  const res = await _test.searchCompaniesHandler(
    makeReq("https://x/api/search-companies?raw=custom%20emblem&norm=custom%20emblem&compact=customemblem&take=25"),
    { log() {} },
    { companiesContainer: container }
  );
  assert.equal(res.status, 200);
  const ids = JSON.parse(res.body).items.map((i) => i.id);
  assert.ok(ids.includes("mainevent"), "emblem-only maker pulled in by head-noun broadening");
  assert.ok(ids.includes("acme"), "full both-word match present");
  assert.ok(ids.indexOf("acme") < ids.indexOf("mainevent"), "full match ranks above head-noun-only maker");
});

test("head-noun broadening is skipped for a brand search (strong name match)", async () => {
  resetTypoCorrectionCache();
  resetIndustryAffinityCache();
  // "blue bottle coffee" has a strong brand match → must NOT be diluted with
  // every coffee company. The head-noun (coffee) query must never fire.
  const brand = { id: "bb", company_id: "bb", company_name: "Blue Bottle Coffee", normalized_domain: "bluebottlecoffee.com", keywords: ["coffee", "espresso"], search_tokens: ["blue", "bottle", "coffee"], search_text_norm: "blue bottle coffee espresso", industries: ["Coffee"], _ts: 1700000000 };
  let headNounFired = false;
  const container = makeContainer(async (spec) => {
    const q = String(spec?.query || "");
    if (q.includes("@hn")) { headNounFired = true; return []; }
    if (q.includes("ARRAY_CONTAINS")) return [brand];
    return [];
  });
  await _test.searchCompaniesHandler(
    makeReq("https://x/api/search-companies?raw=blue%20bottle%20coffee&norm=blue%20bottle%20coffee&compact=bluebottlecoffee&take=25"),
    { log() {} },
    { companiesContainer: container }
  );
  assert.equal(headNounFired, false, "head-noun broadening must not fire for a brand search");
});
