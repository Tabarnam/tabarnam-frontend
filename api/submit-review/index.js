  ...
  if (isEmailConfigured() && !honeypot_tripped) {
    ...
    if (user_email) {
      ...
      const receiptContent =
        row(greeting) +
        row(`Thank you for reviewing <a href="${emailLayout.esc(emailLayout.companyProfileUrl(resolvedCompanyName))}" style="color:#2C7F89;font-weight:700;text-decoration:underline;">${escapeHtml(resolvedCompanyName)}</a> on Tabarnam. We've received your submission, and it's pending approval by our team. You will get another email once it's been reviewed.`);
