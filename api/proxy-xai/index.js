// api/proxy-xai/index.js (classic Node v3 function shape)
const axios = require("axios");
const { CosmosClient } = require("@azure/cosmos");

const BUILD_STAMP = "proxy-xai classic 2025-09-27";
...
module.exports = async function (context, req) {
  ...
  const baseUrl = (vals.FUNCTION_URL || "").trim(); // your dedicated Function URL (must include /api/xai)
  const funcKey = (vals.FUNCTION_KEY || "").trim(); // your app-level key (x-functions-key)
  ...
  const clientId = req.headers?.["x-client-request-id"] || `req_${Math.random().toString(36).slice(2)}_${Date.now()}`;
  const sessionId = String(inbound.session_id || req.headers?.["x-session-id"] || clientId);
  ...
  if (!baseUrl || !funcKey) {
    context.res = {
      status: 500,
      headers: { ...cors(req), "Content-Type": "application/json" },
      body: JSON.stringify({ error: "Server not configured (FUNCTION_URL / FUNCTION_KEY)", meta: { proxy: { status: "misconfigured", build: BUILD_STAMP } } })
    };
    return;
  }
  ...
  try {
    ...
    if (center && expandIfFew && seen.size < desired) {
      expanded = true;
      outer: for (const r of [25, 50, 100]) {
    ...
    context.res = {
      status: e?.code === "ECONNABORTED" ? 504 : 500,
      headers: { ...cors(req), "Content-Type": "application/json" },
      body: JSON.stringify({
        error: "Upstream call failed",
        upstream,
        remoteBody,
        proxy: { status: "error", timeout_ms: timeoutMs, build: BUILD_STAMP, request_id: clientId, baseUrl, session_id: sessionId }
      })
    };
  }
};
