...
async function saveCompaniesToCosmos({ companies, sessionId, requestId, sessionCreatedAt, axiosTimeout, saveStub = false }) {
  try {
    ...
    const preferString = (incoming, existing) => {
    ...
    const preferArray = (incoming, existing) => {
    ...
    const preferNonZeroNumber = (incoming, existing) => {
    ...
    const preferFinite = (incoming, existing) => {
      const inc = Number(incoming);
    ...
    const preferObjectByRecency = (incoming, existing) => {
    ...
    const mergeCompanyDocsForSession = ({ existingDoc, incomingDoc, finalNormalizedDomain }) => {
      const merged = {
        ...existingDoc,
        ...incomingDoc,
      ...
      merged.hq_unknown = mergedHqHasValue
        ? false
        : Boolean(existingDoc.hq_unknown) || Boolean(incomingDoc.hq_unknown);
      merged.hq_unknown_reason = mergedHqHasValue
        ? ""
        : preferString(incomingDoc.hq_unknown_reason, existingDoc.hq_unknown_reason);
      ...
      merged.mfg_unknown = mergedMfgHasValue
        ? false
        : Boolean(existingDoc.mfg_unknown) || Boolean(incomingDoc.mfg_unknown);
      merged.mfg_unknown_reason = mergedMfgHasValue
        ? ""
        : preferString(incomingDoc.mfg_unknown_reason, existingDoc.mfg_unknown_reason);
      ...
      merged.red_flag =
        typeof incomingDoc.red_flag === "boolean" ? incomingDoc.red_flag : Boolean(existingDoc.red_flag);
      ...
      merged.created_at =
        typeof existingDoc.created_at === "string" && existingDoc.created_at.trim()
          ? existingDoc.created_at.trim()
          : incomingDoc.created_at;
