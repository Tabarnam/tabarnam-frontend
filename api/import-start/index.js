...
Return ONLY the JSON array, no other text. Return at least ${Math.max(1, xaiPayload.limit)} diverse results if possible.`;
        ...
        try {
          ...
          // round-trip when the user already provided the URL. The
          // canonical call (resume-worker) handles all the actual
          // research independently; the primary call here only resolves
          // company_name + tagline + structure, all of which the
          // canonical pass refreshes anyway. For the bulk-paste / URL-
          // driven import case, this is pure latency tax.
          //
          ...
          } else if (canSkipPrimaryXaiViaUrl) {
            const urlSeed = buildCompanyUrlSeedFromQuery(query);
            if (urlSeed) {
              try {
                console.log(`[import-start] session=${sessionId} fast_path_url_hint_skip_primary: seed_company_name=${urlSeed.company_name} hint_url=${xaiPayload.company_url_hint}`);
              } catch {}
              mark("xai_primary_fetch_skipped_url_hint");
              xaiResponse = {
                status: 200,
                headers: {},
                data: {
                  choices: [
                    {
                      message: {
                        content: JSON.stringify([urlSeed]),
                      },
                    },
                  ],
                },
              ...
              try {
                ...
                if (e instanceof AcceptedResponseError) {
                  // Match the existing primary-timeout fallback below.
                  const seed = buildCompanyUrlSeedFromQuery(query);
                  addWarning("primary_timeout_company_url", {
                    stage: "primary",
                    root_cause: "upstream_timeout_returning_202",
                    retryable: true,
                    message: "Primary upstream timed out for malformed company_url. Falling back to URL seed.",
                    upstream_status: 202,
                    company_name: seed?.company_name,
                    website_url: seed?.website_url,
                  });
                  ...
                  xaiResponse = {
                    status: 200,
                    headers: {},
                    data: { choices: [{ message: { content: JSON.stringify([seed]) } }] },
                  };
                } else {
                  throw e;
                }
          ...
          if (!dryRunRequested && enriched.length > 0 && cosmosEnabled) {
            ...
            try {
              ...
              if (isCompanyUrlFastPath && mandatoryCompanyIds.length > 0) {
                ...
                // `resume enqueued (1s delay, 1 companies)` log) but the
                // queue trigger function never produced a `handler_entered`
                // event for the session. The most likely cause is a
                // transient Azure Storage Queue drop / poison routing /
                // visibility-timeout race. Without a follow-up message,
                // the import sits at Stub-0% forever (until manual
                // refresh).
                //
                // We enqueue a SECOND message with a 90s delay. If the
                // primary fires successfully, the resume-worker's
                // cycle_count idempotency makes the safety-net's run a
                // no-op (the stub is already enriched, all fields are
                // populated, the worker exits at the missing-fields check
                // with nothing to do). If the primary message was
                // dropped, the safety-net message arrives at T+90s and
                // runs the enrichment that should have happened at T+1s.
                //
                // Net cost: one extra Cosmos read per import (the no-op
                // case at the end of a successful run). Net benefit: lost
                // queue messages stop becoming permanent stub-0% companies.
                try {
                  await enqueueResumeRun({
                    session_id: sessionId,
                    company_ids: mandatoryCompanyIds,
