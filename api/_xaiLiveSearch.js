...
function extractTextFromXaiResponse(resp) {
  ...
  // Phase 2.6 — return empty string when the model emitted no text content.
  // Previous behaviour fell back to JSON.stringify(r), which returned the
  // entire xAI response envelope as if it were the model's text. Downstream
  // parsers (parseCanonicalJson) then succeeded on the envelope and
  // shapeEnrichedFromParsed read empty values for the canonical field
  // names — producing a false-positive success. Returning "" forces callers
  // to explicitly handle the no-text case (model_emitted_no_text in
  // _canonicalImport.js).
  return "";
}
...
async function xaiLiveSearch({
  ...
  response_format, // Optional: { type: "json_schema", json_schema: { name, schema, strict } } for guaranteed shape
} = {}) {
  const configuredModel = asString(
    process.env.XAI_SEARCH_MODEL || process.env.XAI_CHAT_MODEL || process.env.XAI_MODEL || ""
  ).trim();
  const resolvedModel = asString(model).trim() || configuredModel || DEFAULT_XAI_MODEL;
...
async function xaiLiveSearchStreaming({
  ...
  response_format, // Optional: { type: "json_schema", json_schema: { name, schema, strict } } for guaranteed shape
  // Phase 3.3 — replaced Phase 3.0.1's boolean `disablePostCapAbort` with a
  // numeric tolerance. Now caller specifies how many tool calls past the
  // soft cap are allowed before forcing an abort.
  //
  // Phase 3.0.1 set this to "infinity" for multi-call mode (post-cap-abort
  // disabled). Phase 2.19.3's behavior was effectively 0 (abort at first
  // post-cap call). Both were wrong:
  // - Phase 2.19.3 (tolerance 0): Mezzetta/Partanna fired #6 then emitted
  //   text within 15s. Abort killed legitimate synthesis.
  // - Phase 3.0.1 (tolerance ∞): Amagabeli/Camp Chef fired #6..#11 with
  //   ZERO text emitted. Grace timer eventually aborted but burned 40-60s
  //   of dead tool calls + grace per failed sub-call.
  //
  // Sweet spot: tolerance=2 (multi-call) — allows #6, #7, hard-aborts at #8.
  // Catches Mezzetta-style "one more search before emit" while bounding
  // Amagabeli-style deep-research runaway.
  //
  // Single-call mode (Phase 2.x) keeps tolerance=0 (legacy behavior) since
  // its larger 12-cap already has more headroom.
  //
  ...
  // against the 7-9-tool-call satisficing pattern observed in production
  // (5/13 batch: Grillbadger, Grillaholics, HorsePower, Mr. Bar-B-Q,
  // Schmidt Brothers all exited well under the 18-tool budget). Default
  // "high" — counter the model's built-in efficiency bias. Operator
  // override via XAI_REASONING_EFFORT env var ("high" | "medium" |
  // "low" | "off" to disable). When off, no `reasoning` key is sent.
  reasoningEffort,
} = {}) {
  const _resolvedReasoningEffort = (() => {
    const raw = String(reasoningEffort ?? process.env.XAI_REASONING_EFFORT ?? "high").trim().toLowerCase();
    if (raw === "off" || raw === "none" || raw === "" || raw === "false") return null;
    if (raw === "high" || raw === "medium" || raw === "low") return raw;
    return "high"; // unrecognized value defaults to high
  })();
  // Phase 3.3 — reconcile legacy + new param.
  const _effectiveTolerance = disablePostCapAbort
    ? Number.POSITIVE_INFINITY
    : Math.max(0, Math.trunc(Number(postCapToolCallTolerance) || 0));
  const configuredModel = asString(
    process.env.XAI_SEARCH_MODEL || process.env.XAI_CHAT_MODEL || process.env.XAI_MODEL || ""
  ).trim();
  const resolvedModel = asString(model).trim() || configuredModel || DEFAULT_XAI_MODEL;
  ...
  // Phase 2.19.1 — hoisted from inside the try block so they're visible in the
  // catch handler at line ~1042. Previously declared inside the try (lines 736,
  // 759, 760) which made them block-scoped and inaccessible to catch — references
  // in the catch block (e.g. `if (sseStallAborted)`, `tool_calls_counted: toolCalls`,
  // `last_event_silence_ms: Date.now() - lastSseEventAt`) threw ReferenceError.
  // Empirical (Skippy session, 2026-05-09): `streaming_threw { error:
  // 'sseStallAborted is not defined' }` killed the canonical call entirely
  // whenever the tool-cap grace timer fired post-stream-end.
  let toolCalls = 0;
  let lastSseEventAt = Date.now();
  let sseStallAborted = false;
  ...
  try {
    ...
    console.log(`[xaiLiveSearchStreaming] payload_summary`, {
      ...
      timeout_ms: timeoutUsedMs,
      reasoning_effort: _resolvedReasoningEffort || "off",
    });

    // Phase 2.19.6 — single-retry on 503.
    // Empirical (Santa Cruz Organic, 2026-05-09): xAI returned HTTP 503 for
    // a single call; the canonical call completed in 1242ms with all 6
    // fields empty (Stub-0%). 503s on xAI's API are typically transient
    // (1-3s); a one-shot retry with a 2s backoff rescues this case without
    // adding meaningful latency to the success path.
    //
    // We retry ONCE only — multiple 503s in close succession indicate a
    // genuine outage and should fall through to the existing circuit
    // breaker (which opens after 2x 503s in 60s).
    ...
    if (res.status === 503) {
      console.warn(`[xaiLiveSearchStreaming] upstream returned 503 — retrying once after 2s backoff`);
      ...
      try {
        ...
        if (res.ok) {
          console.log(`[xaiLiveSearchStreaming] 503 retry succeeded`);
        } else {
          console.warn(`[xaiLiveSearchStreaming] 503 retry returned status=${res.status} — failing through`);
        }
      } catch (retryErr) {
        // Retry threw (e.g., abort signal fired during sleep). Synthesize a
        // 503-shaped failure so the caller's existing 503 path engages.
        console.warn(`[xaiLiveSearchStreaming] 503 retry threw: ${retryErr?.message || retryErr}`);
        res = { ok: false, status: 503, body: null };
      }
    }

    if (!res.ok) {
      // Phase 4.15 — capture the xAI 4xx/5xx response body so we can
      // diagnose WHY xAI rejected the request. Pre-4.15 we logged
      // `upstream_status: 400` with `text_preview: ''` — no insight into
      // the rejection reason. Empirical (2026-05-16): all canonical calls
      // returning HTTP 400 with zero diagnostic visibility. xAI's 4xx
      // responses typically contain a JSON body like:
      //   {"error": {"code": "invalid_request_error", "message": "..."}}
      // Reading it lets us tell the difference between rate limit,
      // payload contract change, billing flag, or transient service issue.
      //
      // Best-effort: cap body at 2KB to bound log spam if the response
      // is unexpectedly large (e.g. HTML error page from a fronting proxy).
      // Non-fatal: if reading fails, fall back to empty body and continue
      // returning the same failure shape as before.
      let errorBodyPreview = "";
      let errorBodyContentType = "";
      try {
        errorBodyContentType = String(res.headers?.get?.("content-type") || "").toLowerCase();
        // Streaming body may already be partially consumed (it shouldn't be
        // for a non-OK response, but be defensive). Use a small timeout to
        // avoid hanging on a slow body read.
        const readPromise = res.text().catch(() => "");
        const timeoutPromise = new Promise((resolve) => setTimeout(() => resolve(""), 3_000));
        const bodyText = await Promise.race([readPromise, timeoutPromise]);
        errorBodyPreview = String(bodyText || "").slice(0, 2048);
      } catch { /* best-effort */ }

      // Surface the body in a dedicated log line so it's easy to find in
      // production logs. The diagnostics field on the failResult also
      // includes a truncated preview for callers that record diagnostics
      // (resume-worker → enrich_result_summary).
      try {
        console.warn("[xaiLiveSearchStreaming] upstream_4xx_or_5xx_body_captured", {
          upstream_http_status: res.status,
          content_type: errorBodyContentType || "(none)",
          body_chars: errorBodyPreview.length,
          body_preview: errorBodyPreview.slice(0, 800),
          elapsed_ms: Date.now() - startedAt,
        });
      } catch { /* console may be unavailable in test envs */ }
      ...
      const failResult = {
        ...
        diagnostics: {
          ...
          upstream_http_status: res.status,
          streaming: true,
          upstream_error_body_preview: errorBodyPreview.slice(0, 400),
          upstream_error_content_type: errorBodyContentType || null,
        },
      };
    ...
    let buffer = "";
    // toolCalls hoisted to function-level scope above the try (Phase 2.19.1)
    toolCalls = 0;
    let accumulatedText = "";
    const outputItems = [];
    ...
    // Phase 2.9 C — SSE stall detection.
    // Empirical (Earth Runners session bbefd04c, 2026-05-09): cycle 1 of
    // the canonical call streamed 10 web_search_call events in 75 seconds,
    // then went COMPLETELY silent — no further SSE events of any kind for
    // ~3:47 until our 240s timeout fired. Phase 2.5's lock then stayed
    // held until its 5-min TTL elapsed before the next worker could
    // stale-takeover. Net loss: ~5 minutes wall-clock per stalled call.
    //
    // The existing tool-cap grace window (45s) only fires AFTER the model
    // exceeds the tool-call cap. This stall happened well before — model
    // just stopped. So we add a generic "no SSE event for N seconds"
    // detector: bump lastSseEventAt on every parsed event; a separate
    // poller checks every 10s and aborts if the silence exceeds the
    // threshold. Pairs cleanly with Phase 2.5's lock — when SSE stalls,
    // the call aborts within ~60s, the worker's `finally` releases the
    // lock, and the next worker picks up immediately instead of waiting
    // 5 min for the lock TTL.
    // lastSseEventAt and sseStallAborted hoisted to function-level scope above
    // the try (Phase 2.19.1) so the catch block can reference them safely.
    lastSseEventAt = Date.now();
    sseStallAborted = false;
    const SSE_STALL_THRESHOLD_MS = 60_000;  // 60s of silence → assume dead, abort
    const SSE_STALL_CHECK_INTERVAL_MS = 10_000;
    const sseStallChecker = setInterval(() => {
      const silenceMs = Date.now() - lastSseEventAt;
      if (silenceMs > SSE_STALL_THRESHOLD_MS && !sseStallAborted) {
        sseStallAborted = true;
        console.warn(`[xaiLiveSearchStreaming] sse_stall_detected`, {
        ...
        try { clearInterval(sseStallChecker); } catch {}
      }
    ...
    try {
      ...
      while (true) {
        ...
        while ((boundary = buffer.indexOf("\n\n")) !== -1) {
          ...
          // Phase 2.15.B — SSE-stall timer bumps ONLY on meaningful events,
          // not on every parsed event.
          //
          // Empirical (Teva + TKEES, 2026-05-09): xAI sends periodic
          // keep-alive / structural events (`response.in_progress`,
          // `response.created`, etc.) during streams. The Phase 2.9 C
          // stall detector previously bumped `lastSseEventAt` on EVERY
          // parsed event, including these keep-alives, so a model that
          // had silently stopped producing real output but was still
          // receiving xAI keep-alives never tripped the 60s threshold.
          // Result: 5-min lock-TTL waits before the next worker could
          // stale-takeover.
          //
          // Tighten by only bumping on the four event types that
          // genuinely indicate model progress: tool calls, text deltas,
          // output items being added, and final completion. Other events
          // (in_progress / created / unknown) still parse and route
          // normally below — they just don't reset the stall timer.
          //
          // Net: a stalled stream now triggers SSE-stall in 60s even if
          // xAI is sending pings, releasing the lock so the next worker
          // can start its cycle 2 within ~60-70 seconds instead of 5 min.
          const isMeaningfulEvent =
          ...
          if (
            ...
            if (toolCalls >= maxToolCalls && !abortedByToolCap) {
              ...
              // output complete. If no text arrives within the grace window,
              // abort.
              //
              // Phase 2.4: reduced grace window 120s → 45s. Empirical
              // (Birkenstock 150s timeout, 2026-05-08): when the model goes
              // past the tool cap and hasn't started emitting text within
              // ~45s of the cap, it's stuck in a tool loop and won't
              // recover. Aborting at 45s frees the connection for the next
              // worker invocation rather than burning the full timeout on
              // dead grace time.
              //
              // Phase 2.15.A: changed boundary from `>` to `>=`. Empirical
              // (Teva + TKEES, 2026-05-09): the model can stop EXACTLY at
              // the cap (call #10 with cap=10) and never trigger call #11.
              // The old `>` check waited for #11 to arm the grace timer,
              // which never came — the stream then stalled silently for
              // ~3 minutes until the lock TTL elapsed. Firing the grace
              // timer the moment we hit the cap (at call #N where N=cap)
              // catches this stall pattern. The grace window still allows
              // 45s for text output, so legitimate completions at exactly
              // the cap aren't penalized.
              // Phase 3.3 — tightened grace 45s → 25s. Empirical evidence:
              // when the model emits text post-cap legitimately (Mezzetta,
              // Big Spoon), first_text arrives within 0.5-20s. 25s gives
              // generous headroom for that path while saving 20s when the
              // model is genuinely stuck (Amagabeli, Camp Chef).
              console.log(`[xaiLiveSearchStreaming] Tool cap ${maxToolCalls} reached (call #${toolCalls}) — waiting for text output (25s grace)`);
              abortedByToolCap = true;
              setTimeout(() => {
                if (accumulatedText.length === 0) {
                  console.log(`[xaiLiveSearchStreaming] Grace period (25s) expired with 0 text — aborting`);
                  controller.abort();
                }
              }, 25_000);
            } else if (
              ...
              (toolCalls - maxToolCalls) > _effectiveTolerance
            ) {
              // Phase 2.19.3 — aggressive abort when model keeps making tool
              // calls AFTER the cap with still 0 text. Empirical (Adams
              // peanut butter session 2cf0333f, 2026-05-09): cap hit at #12,
              // model then made #13 and #14 during the grace window with 0
              // text emitted, grace expired at +45s with still 0 text →
              // abort. Net loss: 45s of grace burned on the model continuing
              // to research instead of preparing to emit.
              //
              // Compare Big Spoon Roasters (same batch): cap hit at #12 was
              // the LAST tool call; model immediately started emitting text
              // ~0.5s later and finished cleanly at 12 calls / 3685 chars.
              //
              // The signal that distinguishes "stuck" from "wrapping up" is
              // whether ANOTHER tool call fires after the cap is reached.
              // If so, the model is still in research mode and the 45s grace
              // is burning on dead time. Abort immediately and free the lock
              // for the next worker.
              //
              // Phase 3.0.1 — gated behind `disablePostCapAbort` flag.
              // Multi-call mode passes disablePostCapAbort=true because the
              // smaller per-call scope (5-tool budget) puts the model right
              // at the edge of emission when cap is hit; aborting at #6
              // throws away the synthesis. Empirical (Mezzetta + Partanna,
              // 2026-05-10): Locations + Reviews calls fired #6 within 3s
              // of cap and were aborted before the model could emit text.
              // The 45s grace timer (above) is the correct backstop for
              // multi-call.
              console.log(`[xaiLiveSearchStreaming] Tool call #${toolCalls} fired AFTER cap (${maxToolCalls}, tolerance=${_effectiveTolerance}) with 0 text — aborting (model stuck in research loop)`);
              try { controller.abort(); } catch {}
            }
          }
      ...
      // Phase 2.9 C — always clear the SSE-stall checker, even if the loop
      // exits via `break reading`, an exception, or normal completion.
      try { clearInterval(sseStallChecker); } catch {}
    }
    ...
    // Phase 2.9 C — surface sse_stall as a distinct error_code so callers
    // (canonical path) can recognize this specific failure mode in
    // diagnostics. Otherwise it would just appear as upstream_timeout.
    if (sseStallAborted) {
      console.log(`[xaiLiveSearchStreaming] mid-flight aborted by sse stall detector after ${elapsedMs}ms`);
      return {
        ok: false,
        error: "sse_stall",
        ...
        diagnostics: {
          ...
          aborted_by_us: true,
          sse_stall: true,
          last_event_silence_ms: Date.now() - lastSseEventAt,
        },
      };
    }
    return {
      ...
      diagnostics: {
        elapsed_ms: elapsedMs,
        // Phase 2.19.1 hoist enables surfacing the actual tool-call count
        // here. Previously hardcoded to 0, which masked the "model made N
        // tool calls but emitted 0 text → grace timed out" failure pattern.
        tool_calls_counted: toolCalls,
        streaming: true,
        aborted_by_us: Boolean(controller.signal.aborted),
        abort_timer_fired: abortTimerFired,
